...
import { nextTick, reactive, ref, toRaw, watch, computed } from "vue";
...
const taskTitleColor = computed(()=>{
  return setting.theme === 'dark' ? 'red' : ''
})
interface TaskItem {
  id: number | string;
  title: string;
...
const delTask = (tag: TaskItem) => {
  ...
  targetIdx !== -1 && tempVal.splice(targetIdx, 1);
  dynamicTags.value = tempVal;
};
